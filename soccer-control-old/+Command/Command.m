classdef Command < handle
    ...
    methods (Hidden)
        ...
        function traj = buildBodyTraj(obj, init_time, ...
            ...
            iq = mod(init_pos.yaw, 2*pi); fq = mod(fin_pos.yaw, 2*pi);
            if abs(fq - iq) > pi
                if abs(fq) > abs(iq)
                    fq = fq - 2 * pi;
                else
            ...
            traj.data = {
                ...
                Trajectories.BezierTrajectory(duration, ...
                    iq, fq, init_speed.yaw, fin_speed.yaw);
                Trajectories.BezierTrajectory(duration, ...
        ...
        function generateNextFootstep(obj)
            ...
            % Pick the foot to take
            if label == Command.ActionLabel.StanceLeft || ...
                    label == Command.ActionLabel.SwingLeft || ...
                    label == Command.ActionLabel.SwingLeftBack || ...
            ...
            switch(label)
                case {Command.ActionLabel.Forward, Command.ActionLabel.SwingLeft, Command.ActionLabel.SwingRight}
                ...
                case {Command.ActionLabel.Backward, Command.ActionLabel.SwingLeftBack, Command.ActionLabel.SwingRightBack}
                    ninc_pose = obj.actions.positionAtTime(obj.cycle_time / 2 + obj.secant_size);
                    delt_pose = ninc_pose - next_pose;
                ...
                case Command.ActionLabel.Strafe
                    next_q = next_pose.yaw;
                ...
                case Command.ActionLabel.FixStance
                    next_q = next_pose.yaw;
                    ...
                    next_step = [init_pose.x init_pose.y] - normalv/norm(normalv) * step_width;   
                case Command.ActionLabel.Kick
                    footstep = Mechanics.Footstep(...
                        last_step.x - obj.kick_prep_dist * cos(last_step.yaw), ...
                        last_step.y - obj.kick_prep_dist * sin(last_step.yaw), ...
                        0, last_step.side, obj.swing_time ...
                    );
                    obj.footsteps.append(footstep);
                    footstep = Mechanics.Footstep(...
                        footstep.x + obj.kick_forw_dist * cos(last_step.yaw), ...
                        footstep.y + obj.kick_forw_dist * sin(last_step.yaw), ...
                        0, last_step.side, obj.kick_time ...
                    );
                    obj.footsteps.append(footstep);
                    next_step = [last_step.x, last_step.y];
                    next_q = last_step.yaw;
    ...
    methods
        function obj = Command(start_pose)
            ...
            obj.foot_pos = {
                Mechanics.Footstep(-obj.feet_width * sin(start_pose.yaw), ...
                                    obj.feet_width * cos(start_pose.yaw), ...
                                    start_pose.yaw, Mechanics.Foot.Left, 0),... 
                Mechanics.Footstep( obj.feet_width * sin(start_pose.yaw), ...
                                   -obj.feet_width * cos(start_pose.yaw), ...
                                    start_pose.yaw, Mechanics.Foot.Right, 0)
            }; % L, R
            ...
            obj.cur_angles(1,:) = Mechanics.Leg.InverseKinematics(obj.dh, ...
                obj.foot_pos{1}.x + obj.feet_width * sin(start_pose.yaw), ...
                obj.foot_pos{1}.y - obj.feet_width * cos(start_pose.yaw), ...
                -obj.hip_height, 0, obj.cur_angles(1,:));
            obj.cur_angles(2,:) = Mechanics.Leg.InverseKinematics(obj.dh, ...
                obj.foot_pos{2}.x - obj.feet_width * sin(start_pose.yaw), ...
                obj.foot_pos{2}.y + obj.feet_width * cos(start_pose.yaw), ...
        ...
        function [angles, actionState] = next(obj)
            ...
            % Preparation
            if obj.body_traj.isempty()
                if (label == Command.ActionLabel.StanceLeft || ...
                        label == Command.ActionLabel.StanceRight || ...
            ...
            switch(label)
                case Command.ActionLabel.StanceLeft
                    if obj.body_traj.isempty()
                ...
                case Command.ActionLabel.StanceRight
                    if obj.body_traj.isempty()
                ...
                case {Command.ActionLabel.SwingLeft, Command.ActionLabel.SwingLeftBack}
                    obj.foot_traj_l.append(Trajectories.FootCycle( ...
                ...
                case {Command.ActionLabel.SwingRight, Command.ActionLabel.SwingRightBack}
                    obj.foot_traj_l.append(Trajectories.FootCycle( ...
                        obj.actions, obj.foot_pos{1}, obj.foot_pos{1}, ...
                        obj.step_height, -obj.step_outwards, 0, ...
                        footstep.duration ...
                ...
                case Command.ActionLabel.Rest
                    centered_pose = Pose.centeredPose(obj.foot_pos{1}, obj.foot_pos{2});
                    obj.body_traj.append(obj.buildBodyTraj( ...
                        0, 0, obj.body_traj.positionAtTime(0) + action, ...
                        centered_pose, obj.swing_time ...
                    ));
                case {Command.ActionLabel.Forward, Command.ActionLabel.Backward, Command.ActionLabel.Kick}
            ...
            if label == Command.ActionLabel.Forward || label == Command.ActionLabel.Backward
                if (footstep.side == Mechanics.Foot.Left)
            ...
            elseif label == Command.ActionLabel.StanceLeft
                actionState = Command.ActionState.RightToLeftStance;
            elseif label == Command.ActionLabel.StanceRight
                actionState = Command.ActionState.LeftToRightStance;
            elseif label == Command.ActionLabel.SwingLeft
                actionState = Command.ActionState.LeftSwing;
            elseif label == Command.ActionLabel.SwingRight
                actionState = Command.ActionState.RightSwing;
            end
            
            % Transform from world frame to body frame
            abq = action.yaw + bp.yaw;
            ...
            obj.cur_angles(1,:) = Mechanics.Leg.InverseKinematics(obj.dh, ...
                ...
                mod(ftl.yaw - bp.yaw + pi, 2*pi) - pi, ...
                obj.cur_angles(1,:) ...
            );
            obj.cur_angles(2,:) = Mechanics.Leg.InverseKinematics(obj.dh, ...
                rr * cos(rq), rr * sin(rq), ...
                ftr.z - obj.hip_height, ...
                mod(ftr.yaw - bp.yaw + pi, 2*pi) - pi, ...
                obj.cur_angles(2,:) ...
            );
        ...
        function append(obj, label, goal, duration)
            ...
            elseif label == Command.ActionLabel.SwingLeft || ...
                 label == Command.ActionLabel.SwingRight
                duration = obj.cycle_time;
